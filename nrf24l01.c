...
{///
	...
	SPI1_CS_LOW;
	txSPI1(&reg, 1);													// transmit 	
	rxSPI1(&rx, 1);														// receive data
	SPI1_CS_HIGH;
	return rx;
}
...
void rxDataNRF24L01(uint8_t * data)
{
	...
	SPI1_CS_LOW;
	tmp = (NRF24L01_R_RX_PAYLOAD);
	txSPI1(&tmp , 1);												// transmit command to read RX payload 
	rxSPI1(data, (NRF24L01_PIPE_LENGTH));
	SPI1_CS_HIGH;
...
void txPacketNRF24L01(uint8_t * data)
{
	...
	SPI1_CS_LOW;														// clear SS bit
	tmp = NRF24L01_W_TX_PAYLOAD;
	txSPI1(&tmp , 1);													// write cmd to write payload
	txSPI1(data, (NRF24L01_PIPE_LENGTH));
	SPI1_CS_HIGH;
	NRF_CE_HIGH;
